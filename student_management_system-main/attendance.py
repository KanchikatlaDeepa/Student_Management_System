...
class AttendanceClass:
    def __init__(self, root):
        ...
        # ===== Title =====
        Label(self.root, text="Student Attendance Management",
              font=("goudy old style", 24, "bold"),
              bg="#4A90E2", fg="white").pack(fill=X)

        # ===== Input Frame =====
        input_frame = Frame(self.root, bg="#F5F7FA")
        input_frame.pack(fill=X, pady=10)

        Label(input_frame, text="Course:", font=("goudy old style", 14)).grid(row=0, column=0, padx=10)
        self.var_course = StringVar()
        self.txt_course = ttk.Combobox(input_frame, textvariable=self.var_course, state='readonly')
        self.txt_course.grid(row=0, column=1)
        self.txt_course.bind("<<ComboboxSelected>>", self.update_student_list)

        Label(input_frame, text="Date:", font=("goudy old style", 14)).grid(row=0, column=2, padx=10)
        self.date = StringVar()
        DateEntry(input_frame, textvariable=self.date, date_pattern='yyyy-mm-dd').grid(row=0, column=3)

        # ===== Search =====
        Label(input_frame, text="Search:", font=("goudy old style", 14)).grid(row=1, column=0)
        self.search_var = StringVar()
        search_entry = Entry(input_frame, textvariable=self.search_var)
        search_entry.grid(row=1, column=1)
        search_entry.bind("<KeyRelease>", self.search_student)

        # ===== Student Frame =====
        self.student_frame = Frame(self.root)
        self.student_frame.pack(fill=BOTH, expand=True)

        self.canvas = Canvas(self.student_frame)
        self.canvas.pack(side=LEFT, fill=BOTH, expand=True)

        scrollbar = Scrollbar(self.student_frame, command=self.canvas.yview)
        scrollbar.pack(side=RIGHT, fill=Y)
        ...
        Button(btn_frame, text="Submit", bg="green", fg="white",
               command=self.submit_attendance).pack(side=LEFT, padx=5)

        Button(btn_frame, text="All Present",
               command=self.set_all_present).pack(side=LEFT, padx=5)

        Button(btn_frame, text="All Absent",
               command=self.set_all_absent).pack(side=LEFT, padx=5)

        Button(btn_frame, text="Percentage",
               command=self.calculate_percentage).pack(side=LEFT, padx=5)
    ...
    def fetch_courses(self):
        ...
        self.txt_course['values'] = [row[0] for row in data]
        con.close()
    ...
    def update_student_list(self, event=None):
        ...
        cur.execute("SELECT roll, name FROM student WHERE course=?", (course,))
        rows = cur.fetchall()
        ...
        for roll, name in rows:
            ...
            self.student_data[roll] = {'name': name, 'status': var}

            frame = Frame(self.inner_frame)
            frame.pack(fill=X)

            Label(frame, text=roll, width=10).pack(side=LEFT)
            Label(frame, text=name, width=20).pack(side=LEFT)

            Radiobutton(frame, text="P", variable=var, value='P').pack(side=LEFT)
            Radiobutton(frame, text="A", variable=var, value='A').pack(side=LEFT)
    ...
    def search_student(self, event):
        ...
        for roll, data in self.student_data.items():
            if keyword in data['name'].lower() or keyword in str(roll):
                frame = Frame(self.inner_frame)
                frame.pack(fill=X)

                Label(frame, text=roll, width=10).pack(side=LEFT)
                Label(frame, text=data['name'], width=20).pack(side=LEFT)

    # ===== Set All =====
    def set_all_present(self):
        for data in self.student_data.values():
            data['status'].set('P')
    ...
    def submit_attendance(self):
        ...
        for roll, data in self.student_data.items():
            ...
            cur.execute("SELECT * FROM attendance WHERE roll=? AND date=? AND course=?",
                        (roll, self.date.get(), course_id))

            if cur.fetchone():
                cur.execute("UPDATE attendance SET status=? WHERE roll=? AND date=? AND course=?",
                            (status, roll, self.date.get(), course_id))
            else:
    ...
    def calculate_percentage(self):
        ...
        msg = ""
        for roll, present, total in data:
            percent = (present / total) * 100 if total else 0
            msg += f"Roll {roll}: {percent:.2f}%\n"

        messagebox.showinfo("Percentage", msg)
        con.close()
